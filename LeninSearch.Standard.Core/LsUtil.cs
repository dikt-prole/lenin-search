    ...
    public static class LsUtil
    {
        ...
        public static OptimizedFileData LoadOptimized(byte[] lsBytes, CancellationToken ct)
        {
            ...
            for (ushort paragraphIndex = 0; paragraphIndex < paragraphCount; paragraphIndex++)
            {
                ...
                optimizedParagraph.IsHeader = optimizedFileData.Headings?.Any(h => h.Index == paragraphIndex) == true;
                var pageKvp = optimizedFileData.Pages?.FirstOrDefault(kvp => kvp.Key == paragraphIndex);
                if (pageKvp.HasValue)
                {
                    optimizedParagraph.IsPageNumber = true;
                    optimizedParagraph.PageNumber = pageKvp.Value.Value;
                }
