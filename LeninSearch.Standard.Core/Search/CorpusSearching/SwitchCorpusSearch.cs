...
namespace LeninSearch.Standard.Core.Search.CorpusSearching
{
    public class SwitchCorpusSearch : ICorpusSearch
    {
        ...
        public SwitchCorpusSearch(ILsiProvider lsiProvider, int? batchSize, string host, int port, int timeoutMs)
        {
            _lsiProvider = lsiProvider;
            _onlineSearch = new OnlineCorpusSearch(host, port, timeoutMs);
            _offlineSearch = new OfflineCorpusSearch(lsiProvider, batchSize);
        }
