...
namespace LeninSearch.Core
{
    public static class ArchiveUtil
    {
        public static byte[] Save(FileData fileData, Dictionary<string, uint> globalDictionary)
        {
            ...
            if (fileData.Headers?.Any() == true)
            {
                foreach (var h in fileData.Headers)
                {
                    headerBytes.AddRange(BitConverter.GetBytes((ushort)h.Key));
                    var hWords = TextUtil.GetOrderedWords(h.Value);
        ...
        public static FileData Load(byte[] lsBytes, string[] globalDictionary, CancellationToken ct)
        {
            ...
            var localDictionary = new List<string>();
            for (var i = 0; i < wordCount; i++)
            {
                if (ct.IsCancellationRequested) return null;

                var globalIndex = BitConverter.ToUInt32(lsBytes, cursor);
                localDictionary.Add(globalDictionary[globalIndex]);
                cursor += 4;
            ...
            var fileData = new FileData
            {
                ...
                Index = localDictionary.ToDictionary(w => w, w => new List<ushort>()),
                Headers = new Dictionary<ushort, string>(),
            ...
            for (ushort paragraphIndex = 0; paragraphIndex < paragraphCount; paragraphIndex++)
            {
                ...
                {
                    fileData.Paragraphs.Add(string.Empty);
                    continue;
                }
                ...
                {
                    var dictionaryIndex = BitConverter.ToUInt16(lsBytes, cursor);
                    var word = localDictionary[dictionaryIndex];
                    words.Add(word);
                    fileData.Index[word].Add(paragraphIndex);
                    cursor += 2;
                }

                var paragraph = TextUtil.GetParagraph(words);
                fileData.Paragraphs.Add(paragraph);
            }
            ...
            if (pageCount > 0)
            {
                for (ushort i = 0; i < pageCount; i++)
                {
                    ...
                    cursor += 2;
                    fileData.Pages.Add(pageIndex, pageNumber);
                }
            }
            ...
            if (headerCount > 0)
            {
                for (ushort i = 0; i < headerCount; i++)
                {
                    ...
                    for (var j = 0; j < headerLength; j++)
                    {
                        var dictionaryIndex = BitConverter.ToUInt16(lsBytes, cursor);
                        cursor += 2;
                        words.Add(localDictionary[dictionaryIndex]);
                    }
                    var header = TextUtil.GetParagraph(words);
                    fileData.Headers.Add(headerIndex, header);
                }
            }
        ...
        public static FileData Load(string lsFile, string[] dictionary, CancellationToken ct)
        {
            var lsBytes = File.ReadAllBytes(lsFile);

            return Load(lsBytes, dictionary, ct);
        }
