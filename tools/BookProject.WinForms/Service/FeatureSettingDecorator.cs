using System.Threading.Tasks;
using BookProject.Core.Models.Book;
using BookProject.WinForms.YandexVision;

namespace BookProject.WinForms.Service
{
    public class FeatureSettingDecorator : IPageProvider
    {
        private readonly IPageProvider _serviceBase;

        public FeatureSettingDecorator(IPageProvider serviceBase)
        {
            _serviceBase = serviceBase;
        }

        public async Task<(BookProjectPage Page, bool Success, string Error)> GetOcrPageAsync(string imageFile)
        {
            var result = await _serviceBase.GetOcrPageAsync(imageFile);

            if (!result.Success) return result;

            var page = result.Page;

            foreach (var line in page.Lines)
            {
                line.Features = BookProjectFeatures.Calculate(page, line);

                line.Label = line.Features.BelowTopDivider == 0
                    ? BookProjectLabel.Garbage
                    : line.Features.AboveBottomDivider == 0
                        ? BookProjectLabel.Comment
                        : BookProjectLabel.PMiddle;
            }

            return result;
        }
    }
}